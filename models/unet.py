...
class UNetIC(nn.Module):
    def __init__(self, T, num_atr, num_obj, ch, ch_mult, num_res_blocks, dropout, drop_prob=0.1):
        ...
        self.cond_embedding = ConditionalEmbedding(num_atr, ch, tdim, drop_prob)
        self.com_embedding = ConditionalEmbedding(num_obj, ch, tdim, drop_prob)
...
#         )
    ...
    def forward(self, x, timesteps=None, c1=None, c2=None, force_drop_ids=False, **kwargs):
        ...
        context = None
        if self.num_classes is not None and self.num_obj is not None:
...
#                 context_dim = list(context_dim)
        ...
        if self.num_atr is not None:
            if only_table:
                self.label_emb = LabelEmbedding(num_atr, time_embed_dim, drop_prob)
            else:
                self.label_emb = ConditionalEmbedding(num_atr, model_channels, time_embed_dim, drop_prob)
        if self.num_obj is not None:
            if only_table:
                self.atr_emb = LabelEmbedding(num_obj, time_embed_dim, drop_prob)
            else:
                self.atr_emb = ConditionalEmbedding(num_obj, model_channels, time_embed_dim, drop_prob)
