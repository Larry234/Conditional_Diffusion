...
class TripleCondTrainer(nn.Module):
    
    def __init__(self, model: nn.Module, beta: Tuple[int, int], T: int, drop_prob: float = 0.1):
        super().__init__()
        self.model = model
        self.T = T
        self.drop_prob = drop_prob
    ...
    def forward(self, x_0, c1, c2, c3):
        ...
        x_t = (extract(self.signal_rate, t, x_0.shape) * x_0 +
               extract(self.noise_rate, t, x_0.shape) * epsilon)
        epsilon_theta = self.model(x_t, t, c1, c2, c3)
...
class TripleCondDDIMSampler(nn.Module):
    def __init__(self, model, beta: Tuple[int, int], T: int, w: float, schedule="linear"):
        super().__init__()
        self.model = model
        self.T = T
    ...
    def sample_one_step(self, x_t, time_step: int, size: torch.LongTensor, atr: torch.LongTensor, obj: torch.LongTensor, prev_time_step: int, eta: float):
        ...
        epsilon_theta_t = self.model(x_t, t, size, atr, obj)
        unc_epsilon_theta_t = self.model(x_t, t, size, atr, obj, force_drop_ids=True)
